...
public class DijkstraUndirectedSP {
	...
	private boolean multas;// priority queue of vertices
	 ...
	 */
	public DijkstraUndirectedSP(EdgeWeightedGraph G, int s, boolean multas)
	{
		this.multas = multas;
		if(!multas)
		{ 
		...
		else
		{
			...
			{
				if (e.getMultas() < 0)
					throw new IllegalArgumentException("edge " + e + " has negative Multa");
			}
		}
	...
	private void relax(Edge e, int v) {
		...
		else
		{
			...
			int w = e.other(v);
			if (distTo[w] > distTo[v] + e.getMultas()) {
				distTo[w] = distTo[v] + e.getMultas();
				edgeTo[w] = e;
				if (pq.contains(w)) pq.decreaseKey(w, distTo[w]);
				else                pq.insert(w, distTo[w]);
			}
	...
	private boolean check(EdgeWeightedGraph G, int s) {
		...
		for (int v = 0; v < G.V(); v++) {
			for (Edge e : G.adj(v)) {
				...
				{
					if (distTo[v] + e.getMultas() < distTo[w])
					{
						System.err.println("edge " + e + " not relaxed");
						return false;
					}
	...
	public static void main(String[] args) {
		...
		DijkstraUndirectedSP sp = new DijkstraUndirectedSP(G, s, false);
